...
// ✅ TESTING KE LIYE INLINED DUMMY IMAGES HAI, INKO SLIDE1, SLIDE2 SE REPLACE KAR DENA
const bannerSlides = [
  {slide1},
  {slide2},
  {slide3}
...
// ==========================================
// 2. REAL AI STARTUPS DATA (Abhi 10 hai, Future mein yaha badha sakte hain)
// ==========================================
const startupsData = [
  {
    id: 1,
  ...
  },
  {
    ...
    keywords: ['research', 'enterprise']
  },
  {
    id: 8,
    name: 'Scale AI',
    description: 'The Data Infrastructure for AI, annotation platform accelerating AI development with high-quality data.',
    websiteText: 'scale.com',
    url: 'https://scale.com',
    logo: 'https://logo.clearbit.com/scale.com',
    verified: true,
    categories: ['DEVELOPER TOOLS', 'INFRASTRUCTURE', 'RESEARCH'],
    keywords: ['annotation', 'research']
  },
  {
    id: 9,
    name: 'Jasper',
    description: 'Generative AI platform designed to help create content with a focus on marketing.',
    websiteText: 'jasper.ai',
    url: 'https://www.jasper.ai/',
    logo: 'https://logo.clearbit.com/jasper.ai',
    verified: true,
    categories: ['CREATIVE', 'MARKETING', 'PRODUCTIVITY'],
    keywords: ['creative', 'writing']
  },
  {
    id: 10,
    name: 'Synthesia',
    description: 'AI video generation platform allowing businesses to create studio-quality videos in minutes.',
    websiteText: 'synthesia.io',
...
const Startups = () => {
  ...
  // ==========================================
  // 4. SEARCH & CATEGORY FILTER LOGIC (Future proof connection)
  // ==========================================
  const filteredStartups = startupsData.filter((startup) => {
    ...
    const matchesSearch = 
      ...
      (startup.keywords && startup.keywords.some(key => key.toLowerCase().includes(term)));
  ...
  return (
    <div className="min-h-screen bg-[#fcf9f2] font-sans pb-20">
      ...
      {/* ========================================================= */}
      {/* HERO SECTION (Dynamic Local Image Slider)                 */}
      {/* ========================================================= */}
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-20">
            {bannerSlides.map((_, i) => (
              <button 
                ...
                className={`h-2.5 rounded-full transition-all duration-300 ${currentSlide === i ? 'bg-white w-8' : 'bg-white/50 w-2.5 hover:bg-white/80'}`}
              />
            ))}
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto mt-12 px-4 sm:px-6">
        ...
        {/* Search Input (Premium Fully Connected) */}
        <div className="bg-white rounded-full p-2 shadow-sm flex items-center mb-8 border border-gray-100 focus-within:ring-2 focus-within:ring-[#ffcc00] transition-all">
        ...
        {/* Category Filters (Working Logic) */}
        <div className="flex items-center gap-2 overflow-x-auto pb-4 scrollbar-hide">
          ...
          <button className="px-5 py-2.5 rounded-full bg-white text-gray-500 border border-gray-200 text-xs font-bold flex items-center gap-2 hover:bg-gray-50 ml-auto transition-all uppercase whitespace-nowrap">
      ...
      <div className="max-w-6xl mx-auto mt-10 px-4 sm:px-6">
        ...
        <div className="flex justify-between items-end mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-black text-gray-900 tracking-tight uppercase">Ai Startups</h2>
          ...
          <button className="hidden sm:flex bg-white px-4 py-2 rounded-lg border border-gray-200 text-xs font-bold text-gray-600 items-center gap-2 shadow-sm uppercase whitespace-nowrap">
            Newest First
            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
          </button>
        </div>

        {/* Startups Cards List (Mapping logic intact) */}
          ...
          {filteredStartups.length > 0 ? (
            filteredStartups.map((startup, index) => (
              <motion.div
                ...
                className="bg-white rounded-2xl p-5 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.05)] border border-gray-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:shadow-lg transition-all duration-300 group"
              >
                
                {/* Left Side: Logo & Info */}
                <div className="flex items-start gap-5 flex-grow">
                  {/* Company Logo from dataset link */}
                  <div className="w-16 h-16 rounded-2xl bg-white border border-gray-200 shadow-sm flex items-center justify-center flex-shrink-0 p-2 overflow-hidden group-hover:border-[#ffcc00] transition-colors">
                    <img 
                      src={startup.logo} 
                      alt={startup.name} 
                      className="w-full h-full object-contain" 
                      // Fallback logic in case image link fails
                      onError={(e) => { e.target.onerror = null; e.target.src = `https://ui-avatars.com/api/?name=${startup.name}&background=ffcc00&color=000`; }} 
                    />
                  </div>
                  
                  {/* Details */}
                  <div>
                    <h3 className="text-lg font-black text-gray-900 flex items-center gap-1.5 uppercase tracking-wide group-hover:text-[#ffcc00] transition-colors">
                      {startup.name}
                      {startup.verified && (
                    ...
                    <div className="flex flex-wrap gap-2 mt-3 uppercase tracking-wider">
                      {startup.categories.map(cat => (
                        <span key={cat} className="px-2.5 py-1 bg-gray-50 border border-gray-200 text-gray-500 text-[9px] font-bold rounded-md tracking-widest  shadow-sm whitespace-nowrap">
                          {cat}
                        </span>
                      ))}
                    </div>
                ...
                {/* Right Side: Website Link & Prominent Visit Button */}
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 w-full md:w-auto justify-between md:justify-end border-t md:border-none pt-4 md:pt-0 border-gray-100">
                  <div className="flex items-center gap-2 text-gray-400 hover:text-gray-600 text-xs font-bold transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"></path></svg>
                    {startup.websiteText}
                  </div>
                  
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    <button className="p-2.5 text-gray-400 hover:text-[#ffcc00] transition-colors rounded-xl border border-gray-100 hover:border-[#ffcc00] bg-gray-50 hover:bg-yellow-50">
                    ...
                    {/* ✅ PROMINENT & CLEAR VISIT BUTTON - Direct Link Connected */}
                    <a 
                      href={startup.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-grow sm:flex-grow-0 px-8 py-3 rounded-xl bg-[#ffcc00] text-black font-black text-xs hover:bg-black hover:text-[#ffcc00] shadow-md hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-2 tracking-widest uppercase whitespace-nowrap"
                    >
                      Visit Now
